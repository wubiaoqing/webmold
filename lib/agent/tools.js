...
type: 'object',
  ...
  },
  {
    ...
    description:
      '任务完成时调用，输出最终要持久化的定制规则。css 负责外观，js 负责功能，二者至少有一个非空。',
    parameters: {
      type: 'object',
      properties: {
  title: { type: 'string', description: '不超过20字的规则标题' },
        css: { type: 'string', description: '最终 CSS，无则空字符串' },
     js: { type: 'string', description: '最终 JS，无则空字符串' },
        explanation: { type: 'string', description: '用一两句话解释这条规则做了什么' },
        targetRuleId: {
          type: 'string',
          description:
            '若上下文提供了「当前网站已有规则清单」，且你判断本次需求是在修改其中某一条已有规则，则填入该规则的 id（改哪条就填哪条的id）；若本次是新建一条全新规则，则留空或不填。',
        },
      },
