...
function buildSystemPrompt(useNativeTools, registry) {
  const base = `你是一个网页定制 Agent。用户会用自然语言描述想对当前网站做的定制，你要在真实页面上「边看边改」，最终产出可持久化的 CSS/JS 规则。

工作方式：
1. 先用只读工具（query_dom / get_text / get_attributes）探明页面真实结构，找到精准且稳定的选择器。不要凭空猜选择器。
2. 需要验证样式效果时，用 preview_css 临时应用观察。
3. 需要探测运行时行为或验证 JS 逻辑时，用 try_run_js 试跑（它有超时、结果会返回给你）。
4. 当你确信方案可行时，调用 finish 输出最终规则。
...
/**
 * 把用户在页面上「选中的元素」渲染成一段聚焦上下文。
 * 用户显式选择元素通常意味着需求就针对它，模型应优先围绕该元素定位与操作。
 * @param {{selector?:string,tag?:string,id?:string,className?:string,text?:string,outerHTML?:string,rect?:{w:number,h:number}}} [el]
 * @returns {string}
 */
function selectedElementBlock(el) {
  if (!el) return '';
  const lines = ['【用户选中的目标元素】（本次需求很可能就针对它，请优先围绕此元素定位与操作）'];
 ...
 * @param {{url:string,title:string,domSnapshot?:string,selectedElement?:{selector?:string,tag?:string,id?:string,className?:string,text?:string,outerHTML?:string,rect?:{w:number,h:number}}}} params.pageContext
...
export async function runCustomizeAgent({ cfg, userPrompt, pageContext, executors, existingRule, candidateRules, history, signal, onEvent, options = {} }) {
  const useNativeTools = options.useNativeTools !== false; // 默认原生
  ...
  const contextBlock = [
    ...
    selectedElementBlock(pageContext.selectedElement),
  ]
    .filter(Boolean)
    .join('\n');
...
existingRule.title || '未命名规则'
  ...
  // finish 产出完整性校验：不要让“解析失败/被截断”静默退化成空规则。
  // 判断依据：finish 的参数解析不完整（argsOk=false）或输出被 max_tokens 截断（finishReason='length'）。
  ...
  const modelDidTryOutput = !!(meta && meta.rawArgs && meta.rawArgs.length > 20);

  if (bothEmpty && (parseIncomplete || truncated) && modelDidTryOutput) {
    // 模型明明输出了内容，只是没被完整解析出来 —— 这是导致“CSS/JS 均为空，无需保存”的真凶。
    // 直接报错，让用户知道真实原因并可重试/调大最大输出长度，而不是误以为无事发生。
