 ...
 * @property {('start'|'thinking'|'llm_prompt'|'llm_response'|'tool_call'|'tool_result'|'retry'|'finish'|'error')} type
 ...
 * @property {number} [maxNoToolSteps=3]     连续「无工具调用」步数上限：达到后把最后一轮文本兜底为 answer 收尾，
 *                                           避免小模型（本地 Prompt API）无法输出工具调用时陷入死循环
 * @property {number} [maxReadOnlyStreak=4]  连续多次调用同一个只读工具（query_dom/get_text/get_attributes）
 *                                           且参数完全相同的次数上限：达到后判定模型卡在「反复查询」，直接抛错终止，
 *                                           避免小模型从不调用 finish 导致的无限循环
...
export async function runAgent({ chatFn, registry, messages, onEvent = () => {}, options = {} }) {
  ...
  let step = 0;
  let noToolStreak = 0; // 连续「无工具调用」步数，达到 maxNoToolSteps 时强制兜底收尾
  let sameReadOnlyStreak = 0; // 连续调用同一只读工具（参数相同）的次数，达到 maxReadOnlyStreak 时判定卡死
  let lastReadOnlySig = ''; // 上一次只读工具调用的签名
  while (step < opts.maxSteps) {
    throwIfAborted();
    step += 1;
    onEvent({ type: 'thinking', step });

    // 调试观察：把本轮即将发给模型的完整消息序列透出（含 system / 历史 / 工具结果）。
    // 标记 volatile：仅实时推给界面当次展示，不进 task.events、不落历史库（内容体积大）。
    onEvent({ type: 'llm_prompt', step, volatile: true, data: { messages: messages.slice() } });
...
if (!toolCalls || toolCalls.length === 0) {
      ...
      // 连续多轮都解析不出工具调用：小模型（本地 Prompt API）可能始终无法输出约定的
      // JSON 协议。为避免死循环，达到阈值后把最后一轮文本兜底为 answer 收尾。
      noToolStreak += 1;
      if (noToolStreak >= opts.maxNoToolSteps) {
        ...
        const fallbackFinish = {
          ...
          js: '',
          explanation: '',
          answer: content || '抱歉，我无法完成这个任务，请尝试换个说法或使用云端模型。',
        };
        onEvent({ type: 'finish', step, data: fallbackFinish });
        return {
          result: fallbackFinish,
          meta: { argsOk: true, rawArgs: resp.content || '', finishReason: resp.finishReason || '' },
          steps: step,
          messages,
        };
      ...
      messages.push({
        role: 'user',
  content: '请通过调用工具来推进任务；当完成时，调用 finish 工具输出最终结果（mode="rule" 生成规则，或 mode="answer" 直接回答）。',
   });
    continue;
  }
  ...
  noToolStreak = 0; // 出现工具调用，重置连续无工具计数

    // ---- 循环断路器：连续多次调用同一个只读工具且参数完全相同 ----
    // 小模型（本地 Prompt API）常陷入「反复 query_dom 同一个元素、却不调用 finish」的死循环；
    // noToolStreak 因期间夹杂「工具调用」而被反复重置，无法兜底。这里独立统计，
    // 一旦连续超过 maxReadOnlyStreak 次就判定卡死，抛错终止，交给上层提示用户。
    const readOnlySig = detectRepeatedReadOnly(toolCalls);
    if (readOnlySig && readOnlySig === lastReadOnlySig) {
      sameReadOnlyStreak += 1;
    } else {
      sameReadOnlyStreak = readOnlySig ? 1 : 0;
      lastReadOnlySig = readOnlySig || '';
    }
    if (sameReadOnlyStreak >= opts.maxReadOnlyStreak) {
      const err = new Error(
        `模型连续 ${sameReadOnlyStreak} 次重复调用只读工具「${toolCalls[0].name}」且无进展，疑似陷入循环。请尝试简化需求，或切换到云端模型重试。`
      );
      err.code = 'MODEL_STUCK_REPEAT';
...
/** 只读探测类工具：反复调用同一只读工具通常意味着模型卡在「查询」而非推进，属于需要打断的信号 */
const READ_ONLY_TOOLS = new Set(['query_dom', 'get_text', 'get_attributes']);

/**
 * 生成一次「单个只读工具调用」的签名，用于检测模型是否在原地打转。
 * 仅当本次调用恰好是 1 个只读工具时才返回签名，否则返回 null（多工具/写工具不参与此检测）。
 * @param {ToolCall[]} toolCalls
 * @returns {string|null}
 */
function detectRepeatedReadOnly(toolCalls) {
  if (!Array.isArray(toolCalls) || toolCalls.length !== 1) return null;
  const c = toolCalls[0];
  if (!c || !READ_ONLY_TOOLS.has(c.name)) return null;
  try {
...
export function parseTextToolCalls(content) {
  ...
  // 部分本地小模型会误解 responseConstraint，把「约束 schema 定义」本身当作输出吐出来，
  // 形如 {"type":"object","properties":{"tool":{...},...}}。它不是工具调用，返回 null，
  // 让上层走「无工具调用」计数与兜底。
  if (typeof obj.type === 'string' && obj.properties && typeof obj.properties === 'object') {
    return null;
  }
...
function tryExtractFinishFromText(content) {
  ...
  // responseConstraint 下，闲聊/纯提问时模型可能直接用 answer 字段作答（无 tool）
  if (typeof obj.answer === 'string' && obj.answer) {
    return normalizeFinish({ mode: 'answer', answer: obj.answer, explanation: obj.explanation || '' });
  }
  return null;
}
...
function normalizeFinish(a) {
  return {
    mode: a.mode === 'answer' ? 'answer' : 'rule',
